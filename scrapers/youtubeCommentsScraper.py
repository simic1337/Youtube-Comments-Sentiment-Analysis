...
def scrapeCommentsOnVideo(videoID, saveToFile, nextPageToken):
    ...
    with open(saveToFile, 'a', newline='', encoding="utf-8") as file:
        ...
        for index in range(len(response.get('items'))):
            ...
            fileWriter.writerow([response.get('items')[index].get('snippet').get('topLevelComment').get('snippet').get('textDisplay')])
        file.close()
...
if __name__ == "__main__":
    scrapeCommentsOnVideo('XytJ0BchvS8', 'files\\familyFeudVideo.csv', None)